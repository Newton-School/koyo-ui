        ...
        <TabsList className='bg-background h-full flex-col rounded-none p-0'>
          {tabs.map(tab => (
            <TabsTrigger
              key={tab.value}
              value={tab.value}
              className='bg-background data-[state=active]:border-koyo-button-primary data-[state=active]:bg-background data-[state=active]:text-koyo-button-primary h-full w-full justify-start rounded-none border-0 border-l-2 border-transparent data-[state=active]:shadow-none'
            >
              {tab.name}
            </TabsTrigger>
          ))}
