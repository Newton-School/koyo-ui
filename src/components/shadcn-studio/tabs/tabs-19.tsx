        ...
        <TabsList className='bg-background h-full flex-col rounded-none border-l p-0'>
          {tabs.map(tab => (
            <TabsTrigger
              key={tab.value}
              value={tab.value}
              className='bg-background data-[state=active]:border-koyo-button-primary data-[state=active]:bg-background data-[state=active]:text-koyo-button-primary h-full w-full justify-start rounded-none border-l-3 border-transparent data-[state=active]:shadow-none'
            >
              {tab.name}
            </TabsTrigger>
          ))}
