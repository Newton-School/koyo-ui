    ...
    <Accordion type='single' collapsible className='w-full' defaultValue='item-1'>
      {items.map((item, index) => (
        <AccordionItem
          key={index}
          value={`item-${index + 1}`}
          className='data-[state=open]:border-koyo-brand not-last:data-[state=open]:border-b-2'
        >
          <AccordionTrigger className='data-[state=open]:text-koyo-brand [&[data-state=open]>svg]:text-koyo-brand'>
            {item.title}
          </AccordionTrigger>
