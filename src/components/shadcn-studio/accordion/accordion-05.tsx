    ...
    <Accordion type='single' collapsible className='w-full' defaultValue='item-1'>
      {items.map((item, index) => (
        <AccordionItem
          key={index}
          value={`item-${index + 1}`}
          className='data-[state=open]:border-primary not-last:data-[state=open]:border-b-2'
        >
          <AccordionTrigger className='data-[state=open]:text-primary [&[data-state=open]>svg]:text-primary'>
            {item.title}
          </AccordionTrigger>
